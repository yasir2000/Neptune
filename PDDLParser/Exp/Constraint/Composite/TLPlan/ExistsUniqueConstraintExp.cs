...
namespace PDDLParser.Exp.Constraint.TLPlan
{
  ...
  [TLPlan]
  public abstract class AbstractExistsUniqueExp<T> : QuantifiedExp<T>
    where T : class, IExp
  {
    /// <summary>
    ...
    public AbstractExistsUniqueExp(HashSet<ObjectParameterVariable> vars, T exp)
      : base("exists!", vars, exp)
    {
  ...
  /// </summary>
  [TLPlan]
  public class ExistsUniqueConstraintExp : AbstractExistsUniqueExp<IConstraintExp>, IConstraintExp
  {
    /// <summary>
    /// Creates a new uniquely existential expression with the given quantified variables and
    ...
    public ExistsUniqueConstraintExp(HashSet<ObjectParameterVariable> vars, IConstraintExp exp)
      : base(vars, exp)
    {
      System.Diagnostics.Debug.Assert(exp != null && vars != null && !vars.ContainsNull());
    }
    ...
    {
      return new XorUniqueConstraintExp(GetBodySubstitutions().Cast<IConstraintExp>());
    }
